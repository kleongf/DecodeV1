 ...
 */
@Config
@Autonomous(name = "Forward Zero Power Acceleration Tuner Pure Pursuit", group = "Automatic Tuners")
public class ForwardZeroPowerAccelerationTuner extends OpMode {
    ...
    public static double VELOCITY = 30; // this should be set to max velocity
        ...
        if (!end) {
            if (!stopping) {
                if (MathFunctions.dotProduct(poseUpdater.getVelocity(), heading) > VELOCITY) {
                    ...
                    stopping = true;
                    for (DcMotorEx motor : motors) {
                        // we want to decelerate as fast as possible! no zero power bs! let's try -1.
                        motor.setPower(-1);
                    }
                }
            } else {
