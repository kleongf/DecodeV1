...
import org.firstinspires.ftc.teamcode.util.purepursuit2.constants.PPLocalizerConstants;
...
public class PPLocalizer {
    ...
    public PPLocalizer(HardwareMap map) {
        this(map, new Pose());
    }

    public PPLocalizer(HardwareMap map, Pose setStartPose) {
        this.pinpointCooked = false;
        this.hardwareMap = map;
        this.odo = (GoBildaPinpointDriver)this.hardwareMap.get(GoBildaPinpointDriver.class, PPLocalizerConstants.hardwareMapName);
        this.setOffsets(PPLocalizerConstants.forwardY, PPLocalizerConstants.strafeX, PPLocalizerConstants.distanceUnit);
        if (PPLocalizerConstants.useYawScalar) {
            this.odo.setYawScalar(PPLocalizerConstants.yawScalar);
        }

        if (PPLocalizerConstants.useCustomEncoderResolution) {
            this.odo.setEncoderResolution(PPLocalizerConstants.customEncoderResolution);
        } else {
            this.odo.setEncoderResolution(PPLocalizerConstants.encoderResolution);
        }

        this.odo.setEncoderDirections(PPLocalizerConstants.forwardEncoderDirection, PPLocalizerConstants.strafeEncoderDirection);
        this.resetPinpoint();
